...
- `2026-08-03.6`（当前）：51 项，专攻组级记录 —— 汇总值算法（mode/摊平/紧凑写法）、v7→v8 迁移后数字一个不变、每组增删不错位不清表单、打勾按计划铺组、显示/输出/CSV/曲线全走每组明细、merge 不丢 ss；外加一轮冒烟。
...
- **每次改动要顺手更新 `APP_VERSION`**（目前 `"2026-08-03.6"`，用日期串，同一天多次发布加 `.N`）。设置页「检查更新」是 `location.replace(pathname + "?u=" + Date.now())` 绕缓存重载，用户靠版本号确认自己刷到新版了。
