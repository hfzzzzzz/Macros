...
- `2026-08-03.7`（当前）：55 项，在上一轮基础上加了版面断言 —— 每组汇总行随输入更新、「加一组」与下方字段的间距、整张 sheet 全元素两两不重叠。
...
- **每次改动要顺手更新 `APP_VERSION`**（目前 `"2026-08-03.7"`，用日期串，同一天多次发布加 `.N`）。设置页「检查更新」是 `location.replace(pathname + "?u=" + Date.now())` 绕缓存重载，用户靠版本号确认自己刷到新版了。
